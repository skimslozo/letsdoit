...
import dataloader.homogenous as hm
import logging
from dataloader.rigid_interpolation import rigid_interp_split, rigid_interp_geodesic
        ...
        if desired_timestamp in poses_from_traj.keys():
            ...
            if use_interpolation:
                ...
                if abs(float(greater_closest_timestamp) - float(desired_timestamp)) > time_distance_threshold or \
                    ...
                    logging.warning(f'Could not find timestamp within time distance threshold for the desired timestamp, returning.')
                    return None
                ...
                if np.linalg.norm(H0_t - H1_t) > frame_distance_threshold:
                    # print("Skipping frame.")
                    logging.warning(f'Could not find pose within frame distance threshold for the desired timestamp, returning.')
                    return None
