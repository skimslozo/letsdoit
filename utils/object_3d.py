...
class Object3D:
    ...
    def __init__(self, object_instance: ObjectInstance, pcd: open3d.cuda.pybind.geometry.PointCloud, max_points=10000):

        self.max_points = max_points

        self.points = sample_points(np.unique(object_instance.mask_3d, axis=1), n=self.max_points)
        #self.denoise_points(self.points)  # not needed so far
    ...
    def add_object_instance(self, object_instance: ObjectInstance):
        ...
        # self.denoise_points(self.points)  # denoising not needed so far
